...
export default async function handler(req, res) {
	...
	const availablePartsReq = await fetch(
		"https://hackclub.com/api/bin/wokwi/parts",
	);
	...
	const parts = requestedParts.map((requestedPart) => {
		return availableParts.find(
			(availablePart) => availablePart.wokwiName === requestedPart,
		)?.name;
	});
